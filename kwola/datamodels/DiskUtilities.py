    ...
    try:
        ...
        if object is None:
            if printErrorOnFailure:
                print(datetime.now(), "[{}]".format(os.getpid()), "Error: Failed to load object. File not found:", pickleFileName, gzipPickleFileName, jsonFileName, gzipJsonFileName, flush=True)
            return None

        return object
    except json.JSONDecodeError:
        if printErrorOnFailure:
            print(datetime.now(), "[{}]".format(os.getpid()), "Error: Failed to load object {id}. Bad JSON. Usually implies the file failed to write. "
        ...
        if printErrorOnFailure:
            print(datetime.now(), "[{}]".format(os.getpid()), "Error: Failed to load object {id}. Bad pickle file. Usually implies the file failed to write. "
                                  "Sometimes this occurs if you kill the process while it is running. If this occurs "
                                  "during normal operations without interruption, that would indicate a bug.", flush=True)
        return
